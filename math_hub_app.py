...
if user_type == "Learner":
    ...
    # -------------------------
    # LOAD TOPICS ONLY FROM FILES
    # -------------------------
    topic_files = glob.glob("topics/*.py")
    if not topic_files:
        st.warning("No topics available yet!")
    else:
        topics = [os.path.basename(f).replace(".py","").replace("_"," ") for f in topic_files]
        topic = st.sidebar.selectbox("Choose Topic", topics)

        # -------------------------
        # LCM & GCD VISUALIZER
        # -------------------------
        if topic.lower() == "lcm & gcd":
        ...
        # -------------------------
        elif topic.lower() == "prime factors":
        ...
        # -------------------------
        elif topic.lower() == "simultaneous equations":
